...
As we can see here, the standard errors for both $\hat{\alpha}$ and $\hat{\lambda}$ are lower when using Method of Moments Estimation over Maximum Likelihood Estimation. Therefore, we can say that Method of Moments Estimation is better to use for estimating parameters if using this gamma distribution for this specific data. Note how both values are extremely close though and really both seem to do about the same (with Method of Moments ever so slightly better with the lower standard errors).